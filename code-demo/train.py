...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    # print(uv.shape)
    # print(volume)
    # torch.Size([16, 256, 41])
    # torch.Size([16, 641, 41])
    # torch.Size([16, 1, 13120])
    # torch.Size([16, 41])
    # torch.Size([16, 41])
    # print(111)
    # print(emotion.shape)
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q), pred_lf0, norm_lf0, lf0 = net_g(c, f0, uv, spec, g=g, emotion=emotion,mel=mel)
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_gen_all = loss_gen + loss_fm + loss_mel + loss_kl + loss_subband#+ loss_lf0#+pitch_loss#+ loss_dur
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
      for batch_idx, items in enumerate(eval_loader):
        ...
        f0 = f0[:1].cuda(0)
        uv = uv[:1].cuda(0)
        emotion=emotion[:1].cuda(0)
        if c.shape[2]<=spec.shape[2]:
            ll = c.shape[2] // 4
            c = c[:, :, :ll * 4]
            spec = spec[:, :, :ll * 4]
            f0 = f0[:,:ll*4]
      ...
      y_hat = generator.module.infer(c,f0, uv, g=g, emotion=emotion, mel1=mel,mel=mel)
      mel = spec_to_mel_torch(
          spec, 
          hps.data.filter_length,
          hps.data.n_mel_channels, 
    ...
    y_gt=y*32768
    print(y_hat.size())
    torchaudio.save("temp_result/vctkms_new_tem_result_{}.wav".format(global_step),y_hat[0, :, :].cpu(),16000)
    torchaudio.save("temp_result/vctkms_new_tem_result_gt_{}.wav".format(global_step),y[0, :, :].cpu(),16000)
