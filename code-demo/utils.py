...
def load_checkpoint(checkpoint_path, model, optimizer=None):
  ...
  if optimizer is not None:
    print("7777777777777")
    # 如果有优化器状态，也加载它
    #optimizer.load_state_dict(checkpoint_dict.get('optimizer', {}))  # 使用 get 来避免 KeyError
    #print("888888888882222222222")
  saved_state_dict = checkpoint_dict['model']
  ...
  print("444444444444")
  # 只加载匹配的参数
  new_state_dict = {k: v for k, v in saved_state_dict.items() if k in state_dict and v.size() == state_dict[k].size()}

  # 打印出被加载的层
  print("\nLoaded layers from checkpoint:")
  for k in new_state_dict:
    print(k)

  # 打印出未被加载的层
  print("\nSkipped layers not in checkpoint:")
  for k in state_dict:
